...
// TestHandleListTerminalsEmpty verifies that HandleListTerminals returns an empty list
// when no terminals are open.
func TestHandleListTerminalsEmpty(t *testing.T) {
	deps := &TerminalDeps{
		Pool:     connection.NewPool(&config.SSHConfig{}, nil),
		TermPool: connection.NewTerminalPool(0),
		Config:   &config.SSHConfig{AllowTerminal: true},
	...
	out, err := HandleListTerminals(context.Background(), deps, SSHListTerminalsInput{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if out.Count != 0 {
		t.Errorf("expected 0 terminals, got %d", out.Count)
	}
	if len(out.Terminals) != 0 {
		t.Errorf("expected empty terminals slice, got %d entries", len(out.Terminals))
	}
	text := out.Text()
	if text != "No active terminals" {
		t.Errorf("expected 'No active terminals', got %q", text)
	}
}

// TestHandleListTerminalsWithFilter verifies that HandleListTerminals respects the session filter.
func TestHandleListTerminalsWithFilter(t *testing.T) {
	deps := &TerminalDeps{
		Pool:     connection.NewPool(&config.SSHConfig{}, nil),
		TermPool: connection.NewTerminalPool(0),
		Config:   &config.SSHConfig{AllowTerminal: true},
	...
	// Insert synthetic terminals directly into the pool.
	tp := deps.TermPool
	for _, id := range []string{"term-1", "term-2"} {
		ts := &connection.TerminalSession{
			ID:        connection.TerminalID(id),
			SessionID: connection.SessionID("user@host:22"),
		}
		tp.InsertForTest(ts)
	}
	...
	if out.Count != 2 {
		t.Errorf("expected 2 terminals, got %d", out.Count)
	}
	...
	if out.Count != 2 {
		t.Errorf("expected 2 terminals for matching session, got %d", out.Count)
	}

	// List filtered by non-matching session.
	out, err = HandleListTerminals(context.Background(), deps, SSHListTerminalsInput{
		SessionID: "other@host:22",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if out.Count != 0 {
		t.Errorf("expected 0 terminals for non-matching session, got %d", out.Count)
	}
}
